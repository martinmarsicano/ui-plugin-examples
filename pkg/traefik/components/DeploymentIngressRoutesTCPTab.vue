...
export default {
  ...
  async fetch() {
    const [ingressRoutesTCP, services] = await Promise.all([
      this.$store.dispatch('cluster/findAll', { type: 'traefik.io.ingressroutetcp' }),
      this.$store.dispatch('cluster/findAll', { type: 'service' }),
    ]);

    this.allIngressRoutesTCP = ingressRoutesTCP;
  ...
  computed: {
    ...
    relatedIngressRoutes() {
      ...
      return this.allIngressRoutesTCP
