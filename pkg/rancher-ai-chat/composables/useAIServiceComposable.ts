...
export function useAIServiceComposable() {
  ...
  function decodeLLMConfig(configMapData: any): LLMConfig {
    ...
    return {
      name:  activeLlm ? t(`aiChat.configurations.models.${ activeLlm }`) : '',
      model: activeModel
    };
  }
  ...
  async function fetchLLMConfigs() {
    if (!hasPermissions.value) {
      error.value = {
        key:         'aiChat.error.services.settings.noPermission',
        sourceLinks: [{
          label: t('aiChat.error.services.docsLinkLabel'),
    ...
    if (!configMap) {
      error.value = { key: 'aiChat.error.services.settings.missingConfig' };
    ...
    if (!config.name) {
      error.value = { key: 'aiChat.error.services.settings.missingActiveLLM' };

      return;
    }

    if (!config.model) {
      error.value = { key: 'aiChat.error.services.settings.missingModel' };
  ...
  watch(() => aiAgentDeploymentState.value, (newState) => {
    if (newState === AIServiceState.NotFound) {
      error.value = { key: 'aiChat.error.services.deployment.notFound' };
    } else {
      error.value = null;
