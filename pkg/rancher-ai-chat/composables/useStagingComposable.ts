...
export function useStagingComposable() {
  ...
  function open(args: OpenArgs) {
    ...
    if (store.state.$router.currentRoute.value.name === `c-cluster-${ PRODUCT_NAME }-staging`) {
      currentRoute = store.getters['rancher-ai-chat/staging/all']?.previousRoute || '';
    }

    store.commit('rancher-ai-chat/staging/setData', {
      ...args,
      route: currentRoute,
    });
