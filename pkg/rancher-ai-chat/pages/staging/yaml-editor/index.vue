...
const resourceLabel = computed(() => {
  if (!props.value?.resource) {
    return t('aiChat.staging.yaml-editor.unknownResource', {}, true);
  }
...
<template>
  <div
    ...
    data-testid="rancher-ai-chat-staging-yaml-editor"
  >
    <div class="staging-yaml-header">
      <div class="staging-yaml-title">
        <h1>{{ props.value?.title || t('aiChat.staging.yaml-editor.title', {}, true) }}</h1>
      ...
      >
        <RcButton
          secondary
          data-testid="rancher-ai-chat-staging-yaml-editor-cancel-button"
          @click="handleCancel"
        >
          {{ t('aiChat.staging.yaml-editor.cancel', {}, true) }}
        </RcButton>
        <RcButton
          primary
          data-testid="rancher-ai-chat-staging-yaml-editor-apply-button"
          @click="handleApply"
        >
          {{ t('aiChat.staging.yaml-editor.apply', {}, true) }}
        </RcButton>
      </div>
      <div
        v-else
      ...
      >
        <RcButton
          secondary
          data-testid="rancher-ai-chat-staging-yaml-editor-close-button"
          @click="emit('close')"
        >
          {{ t('aiChat.staging.yaml-editor.close', {}, true) }}
        </RcButton>
      </div>
    </div>
    ...
    <div class="staging-yaml-editor-container">
      <YamlEditor
        ...
        :scrolling="true"
        data-testid="rancher-ai-chat-staging-yaml-editor-code-mirror-editor"
        class="yaml-editor"
      />
    </div>
  </div>
