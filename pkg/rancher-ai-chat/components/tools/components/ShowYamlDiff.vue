...
const label = computed(() => {
  ...
  return props.tool?.input.resourceName || t(`aiChat.tools.${ props.tool.toolName }.name`, { }, true);
});
...
const tooltip = computed(() => {
  ...
  return t(`aiChat.tools.${ props.tool.toolName }.tooltip`, {
    kind,
    namespace,
    name
  }, true);
  ...
  <div v-if="isValidInput">
    ...
    >
      <RcButton
        small
        tertiary
        :data-testid="`rancher-ai-chat-tool-show-yaml-diff-${ props.tool.input.resourceKind }-${ props.tool.input.resourceNamespace }-${ props.tool.input.resourceName }`"
