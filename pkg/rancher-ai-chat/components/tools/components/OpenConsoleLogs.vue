...
<template>
  <div v-if="!!pod">
    <RcButton
      small
      tertiary
      :data-testid="`rancher-ai-chat-tool-open-console-logs-${ props.tool.input.cluster }-${ props.tool.input.namespace }-${ props.tool.input.name }-${ props.tool.input.containerName }`"
    ...
    >
      <div class="open-console-logs-tool-label">
        ...
        >
          {{ t(`aiChat.tools.${ props.tool.toolName }.name`, {}, true) }}
        </span>
      </div>
    </RcButton>
  </div>
