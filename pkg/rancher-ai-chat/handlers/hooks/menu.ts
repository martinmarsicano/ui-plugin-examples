    ...
    if (context.value && typeof context.value === 'object') {
      ...
      if (obj.stateDescription && obj.state !== 'active') {
        items.push({
          ...
          label:    t(`aiChat.hooks.menu.itemMessages.troubleshoot`, {
            kind: obj.kind || '',
            name: obj.name || ''
          }, true)
        });
        items.push({
          ctx,
          labelKey: 'recover',
          label:    t(`aiChat.hooks.menu.itemMessages.recover`, {
            kind: obj.kind || '',
            name: obj.name || ''
          }, true)
        });
      ...
      items.push({
        ctx,
        labelKey: 'inspect',
        label:    t(`aiChat.hooks.menu.itemMessages.inspect`, {
          kind: obj.kind || '',
          name: obj.name || ''
        }, true)
      });
    ...
    if (context.value && typeof context.value === 'string') {
      if (context.tag === '__sortable-table-row-description') {
        items.push({
          ctx:      [],
          labelKey: 'explain',
          label:    t(`aiChat.hooks.menu.itemMessages.explain`, { error: context.value || '' }, true)
        });
      }
    }
  ...
  build(store: Store<any>, context: Context): HTMLElement {
    ...
    btn.innerHTML = `
      ...
      >
        ...
        >
          ...
          >
            ${ t('aiChat.hooks.menu.header.title') }
          </span>
        </div>
    ...
    menuItems.forEach(({ labelKey }) => {
      btn.innerHTML += `
        ...
        >
          ${ t(`aiChat.hooks.menu.items.${ labelKey }`) }
        </button>
      `;
    });
    ...
    menuItems.forEach(({ labelKey, label, ctx }) => {
      ...
      if (element) {
        element.addEventListener('click', () => {
          Chat.open(store);
          nextTick(() => {
            store.commit('rancher-ai-chat/input/text', label);

            store.commit('rancher-ai-chat/context/reset');
            if (ctx && ctx.length) {
              store.commit('rancher-ai-chat/context/add', ctx);
            }
